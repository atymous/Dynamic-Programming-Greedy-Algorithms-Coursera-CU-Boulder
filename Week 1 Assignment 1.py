...
def maxSubArray(a):
    ...
    if n == 1:
        return 0
    # your code here
    # initialize minSoFar(+infinity) and maxDiff (difference, -infinity)
    minSoFar = float('inf')
    maxDiff = float('-inf')
    
    # iterate rate through n and return the maxDiff
    for i in range(n):
...
def polynomial_multiply(a_coeff_list, b_coeff_list):
    ...
    # For this function, initialize both parameters/polynomials as a list
    x = len(a_coeff_list)
    y = len(b_coeff_list)
    
    # include padding for formatting output
    padded_length = (x + y - 1)
    
    # initialize new padded strings
    a_padded = a_coeff_list + [0] * (padded_length - x)
    b_padded = b_coeff_list + [0] * (padded_length - y)
    
    # compute fft
    fft_a = fft(a_padded)
    fft_b = fft(b_padded)
    
    # use numpy packages in python to performe multiplcation
    fft_result = [fft_a[i] * fft_b[i] for i in range(padded_length)]
    
    # return the real component of a complex number
    result_coeff = [real(x) for x in ifft(fft_result)]
    
    # return the final coefficent list!
    return result_coeff
...
def check_sum_exists(a, b, c, n):
    ...
    # use the result to solve the problem at hand
     ...
     # initialize coefficients for the polynomial parameters
    a_coeffs = [0]*n
    b_coeffs = [0]*n 
    ...
    # multiply both polynomials using the "polynomial_multiply" attribute
    ab_mult_result_coeffs = polynomial_multiply(a_coeffs, b_coeffs)
    
    # iterate through the list for a coefficient in the result above that corresponds to parameter 'c'
    # if no matches are found return False!
    for num in c:
        if ab_mult_result_coeffs[num] >= 0.5:
            return True
...
# do not include the plot/labels from the example above
...
# code here! use fft on the weekly prices list and save that as a new list
fft_data = fft(weekly_prices)
N = len(fft_data)
# ensure proper conditions using assert
...
# TODO: go through the fft_data and select only those frequencies in the range lo/hi
    new_fft_data = []
    for (fft_val, fft_freq) in zip(fft_data, fft_frequencies):  # make sure we have the 0 frequency component
...
# here are the 3 lists from the above example
...
less_than_1_quarter = select_all_items_in_freq_range(1/13, 1) # All signal components with frequency >= 1/13 
